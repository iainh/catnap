...
```toml
catnap = { version = "0.4", features = ["xml"] }
```
...
```toml
catnap = { version = "0.4", default-features = false }
```

JSON without basic auth or TLS:

```toml
catnap = { version = "0.4", default-features = false, features = ["json"] }
```

JSON with HTTPS but without basic auth:

```toml
catnap = { version = "0.4", default-features = false, features = ["json", "tls-rustls"] }
```
